...
export const useTableElement = () => {
      ...
      for (let i = 0; i < 4; i++) {
        for (let j = 0; j < 4; j++) {
          ...
          const text = new AutoResizingTextbox(``, {
            ...
            selectable: true,
            backgroundColor: i === 0 ? '#7fa7dc' : 'transparent',
            name:
              i === 0
                ? `${TABLE_HEADER}_${i + 1}_${j + 1}`
                : `${TABLE_TEXT}_${i + 1}_${j + 1}`,
  ...
  const removeTableHeader = (canvas: fabric.Canvas) => {
    const activeObject = canvas.getActiveObject();
    if (!activeObject || !(activeObject instanceof fabric.Group)) return;
    ...
    canvas.getObjects().forEach(obj => {
      if (obj.name && obj.name.startsWith(`${TABLE_HEADER}_${1}`)) {
        canvas.remove(obj);
      }
    });

    activeObject.forEachObject(obj => {
      if (obj.name && obj.name.startsWith(`${TABLE_CELL}_${1}`)) {
        activeObject.removeWithUpdate(obj);
      }
    });
  ...
  const addTableHeader = (canvas: fabric.Canvas) => {
    const activeObject = canvas.getActiveObject();
    if (!activeObject || !(activeObject instanceof fabric.Group)) return;
    ...
    if (activeObject?.type == 'group' && activeObject.name?.startsWith(TABLE)) {
      for (let i = 1; i <= Number(col); i++) {
        ...
        const text = new AutoResizingTextbox(``, {
          ...
          top: cell.top! + 6,
          selectable: true,
          backgroundColor:'#7fa7dc',
          name: `${TABLE_HEADER}_${1}_${i+1}`,
          fixedWidth: 150 - 6,
          fixedHeight: 50 - 6,
          lockMovementX: true,
          lockMovementY: true,
